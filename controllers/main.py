...
class GatewayApiWebhookController(http.Controller):
    ...
    def gatewayapi_dlr_webhook(self, **kwargs):
        ...
        if not jwt_secret:
            _logger.error(
                "GatewayAPI DLR: JWT secret not configured in system parameters"
            )
            return Response(
                json.dumps({
                    'status': 'error',
        ...
        try:
            ...
            return Response(
                json.dumps({
                    ...
                    'message': (
                        'Invalid token. ' +
                        'Please check GatewayAPI configuration and JWT secret.'
                    )
                }),
                status=403,
                mimetype='application/json'
        ...
        if not all(k in data for k in ['id', 'status']):
            _logger.warning("GatewayAPI DLR: Missing required fields in payload. "
                            "Data: %s", data)
            return Response(
                json.dumps({
                    'status': 'error',
        ...
        if new_odoo_state != original_odoo_state or failure_type != sms_message.failure_type:
            sms_message.write({
