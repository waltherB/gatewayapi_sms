...
class IapAccount(models.Model):
    ...
    # Obsolete methods related to channel notifications (like _check_channel_name_required, 
    # _onchange_iap_account_name_for_channel, _compute_effective_notification_channel, 
    # _compute_field_exists, _create_gatewayapi_new_channel) have been removed.
    ...
    def write(self, vals):
        ...
        if 'gatewayapi_check_min_tokens' in vals:
            for record in self.filtered(lambda r: r.is_gatewayapi):
                if record.gatewayapi_check_min_tokens and not record.gatewayapi_token_notification_action:
                    try:
                        action = self.env.ref('gatewayapi_sms.low_credits_notification_action')
                        record.gatewayapi_token_notification_action = action.id
                    except Exception as e:
                        _logger.error(f"Failed to set notification action for {record.id}: {e}")
        return res
